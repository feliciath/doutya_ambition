...
import PostCommentStructure from "./(components)/PostCommentStructure";
import { IoSend } from "react-icons/io5";
import PostReplyStructure from "./(components)/PostReplyStructure";
import { useAppSelector } from "@/lib/hooks";

const CommentsPage = ({ params }) => {
  const user = useAppSelector((state) => state.auth.user);

  const post_id = params.post_id[0];
  const page_id = params.post_id[1];
    ...
    if (page_id) {
      try {
        const response = await axios.get(
          `${baseURL}/getPostComments.php?post_id=${post_id}&userId=${user?.id}`
        );
